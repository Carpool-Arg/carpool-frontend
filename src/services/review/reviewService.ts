...
export async function canDriverReview(tripId: string, passengerId: string): Promise<BooleanResponse>{
  
  try{
    const res = await fetch(`/api/review/can-driver-review/${tripId}/${passengerId}`,{
      headers: {
        "Content-Type": "application/json",
      },
      credentials: 'include'

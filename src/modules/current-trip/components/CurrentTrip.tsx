...
export default function CurrentTrip() {
  const {currentTrip, arriveNextStop } = useTrip();
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="bg-gray-8/90 backdrop-blur rounded-xl p-6 space-y-4">
            ...
            <div className="flex items-center justify-end pt-2">
              {nextStop && 
                <button 
                  className="bg-gray-6 text-gray-8 px-4 py-2 rounded-lg font-medium cursor-pointer"
                  onClick={arriveNextStop}
                >
                { nextStop ? `Llegué a ${capitalizeWords(nextStop?.tripStop.cityName)}` : isLastStop
                  && "Finalizar viaje" 
                }
                </button>
              }
